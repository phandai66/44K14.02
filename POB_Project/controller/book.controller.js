...
module.exports.postNewBook = async (req, res) => {
    ...
    const bookExisted = await Book.findOne({ bookName })
    if (bookExisted) {
        res.render('layouts/book-form-new', { username : acc, message: 'This name already existed!' })
        return
    } else {
...
//Load ra trang quản lý kho sách
module.exports.getBooksForEdit = async (req, res) => {
    const acc = await Currentuser.getCurrentUser(req, res) //Lấy ra người dùng hiện tại
    const books = await Book.find({}) 
...
module.exports.showMainPage = async (req, res) => {
    ...
    const { username } = req.user //lấy ra người dùng hiện tại
    res.render('layouts/main', { username }) // { username } là để in tên người dùng ở góc trên bên phải giao diện
    return
}

//cái này là sau khi tìm kiếm sách bởi admin, load trang này ra
module.exports.getBooksDetailsByName = async (req, res) => {
    const { username } = req.user
    const acc = await accountModel.findOne({ username })

    //nếu không gõ gì mà nhấn tìm kiếm thì sẽ load ra trang chủ của admin
    if (req.query.name == '') {
        res.redirect('/admin')
    } else {
